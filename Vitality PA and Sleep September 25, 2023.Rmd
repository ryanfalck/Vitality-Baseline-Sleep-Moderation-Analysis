...
Vitality <- read_excel("VITALITY_MASTER_FINAL_SLEEP_May16,2023.xlsx")
```
...
Sleep_data<- subset(data4, !is.na(MW_Duration) | !is.na(MW_Efficiency) | !is.na(MW_Latency) |
                      !is.na(MW_Fragmentation) | !is.na(MW_Awakenings))
```
...
Sleep_data_2<-Sleep_data %>%
  ...
  mutate(Good_Sleep_Quality = as.factor(ifelse(Time==1 & PSQI<=5,1,0)),
         Good_Sleep_Duration = as.factor(ifelse(Time==1 & MW_Duration>=420,1,0)),
         Good_Sleep_Efficiency = as.factor(ifelse(Time==1 & MW_Efficiency>=85,1,0)),
         Good_Sleep_Fragmentation = as.factor(ifelse(Time==1 & MW_Fragmentation<=median(MW_Fragmentation),1,0)),
         Good_Sleep_Latency = as.factor(ifelse(Time==1 & MW_Latency<=median(MW_Latency),1,0))
         )
```

# 2) Baseline Participant Characteristics by group and sleep
...
GoodSleepFrag<-subset(Baseline, Good_Sleep_Fragmentation==1)
BadSleepFrag<-subset(Baseline, Good_Sleep_Fragmentation==0)
GoodSleepLat<-subset(Baseline, Good_Sleep_Latency==1)
BadSleepLat<-subset(Baseline, Good_Sleep_Latency==0)

vars<-dput(names(Baseline[c(31,32,34,72,4,6,7,36,37,23,24,55,54,14,71,75,39,79,51,61,56:59)]))
```
...
Participants categorized by subjective sleep quality (i.e., Good subjective sleep PSQI ≤ 5)
```{r}
...
Participants categorized by sleep duration (i.e., Good sleep duration ≥ 420 minutes/night)
```{r}
...
Participants categorized by sleep efficiency (i.e., Good sleep efficiency ≥ 85%/night)
```{r}
...
# 3) Primary Linear Mixed Model (From Liu-Ambrose et al., 2022)

Prepare data for linear mixed model
```{r}
data5<-Sleep_data_2[c(1:3,81:85,31,32,34,72,4,6,7,36,37,23,24,55,54,14,71,75,39,79,51,61,56:59)]

varying<-data5[c(1:2,22:32:length(data5))]
baseline<-subset(data5[-c(22:32:length(data5))], Time==1)
...
data7 <- reshape(as.data.frame(wide.data),idvar="ID",varying=32:53,direction="long",sep=".") #reshape to long data frame with T2 and T3 repeated outcome and baseline as separate time-invariant
...
## 3.1) Primary Analysis: ADAS-Cog Plus
```{r}
lmm.ex.mdl("ADASCOGPLUS", "ADASCOGPLUSbaseline", "MMSE", "FMTotalScore")#PRIMARY OUTCOME:ADAS-COG Plus
```

## 3.2) Secondary Outcome: ADAS-Cog 13
```{r}
lmm.ex.mdl("ADASCObaseline1", "ADASCObaseline1baseline", "MMSE", "FMTotalScore")#SECONDARY OUTCOME: Stroop Interference Ratio
```

## 3.3) Secondary Outcome: SPPB
```{r}
lmm.ex.mdl("SPPBScore", "SPPBScorebaseline", "MMSE", "FMTotalScore")#SECONDARY OUTCOME: Short Physical Performance Battery
```

## 3.4) Secondary Outcome: Gait Speed
```{r}
lmm.ex.mdl("GaitSpeed", "GaitSpeedbaseline", "MMSE", "FMTotalScore")#SECONDARY OUTCOME: Gaitspeed
```


# 4) Linear mixed model for comparing outcomes based on subjective sleep quality (Good sleep quality PSQI ≤ 5)
...
```{r}
lmm.ex2.mdl("ADASCObaseline1", "ADASCObaseline1baseline", "MMSE", "FMTotalScore")#SECONDARY OUTCOME: Stroop Interference Ratio
```
...
# 5) Linear mixed model for comparing outcomes based on objective sleep duration (Good sleep duration ≥ 420 minutes/night)
...
```{r}
lmm.ex3.mdl("ADASCObaseline1", "ADASCObaseline1baseline", "MMSE", "FMTotalScore")#SECONDARY OUTCOME: Stroop Interference Ratio
```
...
# 6) Linear mixed model for comparing outcomes based on objective sleep duration (Good sleep duration ≥ 85%/night)
...
```{r}
lmm.ex4.mdl("ADASCObaseline1", "ADASCObaseline1baseline", "MMSE", "FMTotalScore")#SECONDARY OUTCOME: Stroop Interference Ratio
```
